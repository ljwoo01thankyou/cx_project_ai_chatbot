...
const questions = [
  {
    ...
    subtext: '가격 / 성능 / 디자인의 우선순위를 차례대로 선택해주세요',
    rankingItems: ['가격', '성능', '디자인'],
    inputPlaceholder: '선택지에 없다면 중요하게 생각하는 구매 기준을 입력해 주세요.',
  },
  {
    id: 1,
    text: (
  ...
  },
  {
    ...
    inputPlaceholder:
      '선택지에 없다면, 현재 집에 몇 명이 함께 살고 있는지 입력해 주세요.',
  },
  {
    id: 3,
    text: '구매 목적이 무엇인가요?',
...
export default function QuestionsPage() {
  ...
  const isRankingQuestion = current.type === 'ranking';
    ...
    if (currentQuestion < questions.length - 1) {
      ...
      setTimeout(() => {
        navigate('/loading-seller', {
          state: {
            answers: newAnswers,
            familyType: newAnswers[2],
            purchasePurpose: newAnswers[3],
          },
        });
      }, 500);
    }
  ...
  const handleRankingSelect = (item: string) => {
    if (rankingSelection.includes(item)) return;
  
    const nextSelection = [...rankingSelection, item];
    setRankingSelection(nextSelection);
  
    if (nextSelection.length === 3) {
      const rankingAnswer = nextSelection.join(' > ');
      const newAnswers = [...answers];
      newAnswers[currentQuestion] = rankingAnswer;
      setAnswers(newAnswers);
                ...
                >
                  ...
                  <div className="bg-white/60 backdrop-blur-xl rounded-3xl p-8 border border-white/50 shadow-xl">
                    ...
                    {isRankingQuestion && (
                      <div className="mt-5 flex flex-wrap gap-2">
                        {rankingSelection.map((item, index) => (
                          <div
                            key={item}
                            className="px-4 py-2 bg-purple-100/70 backdrop-blur-sm text-purple-700 rounded-full text-sm border border-purple-200/50"
                          >
                            {index + 1}순위 · {item}
                          </div>
                        ))}
                  ...
                  {isRankingQuestion ? (
                    <div className="grid sm:grid-cols-3 gap-4">
                      {current.rankingItems.map((item, index) => {
                        const selectedIndex = rankingSelection.indexOf(item);
                        ...
                        return (
                          <motion.button
                            ...
                            disabled={isSelected}
                            className={`group relative px-5 py-5 min-h-[96px] rounded-2xl border transition-all shadow-lg hover:shadow-xl text-left ${
                              isSelected
                                ? 'bg-gradient-to-r from-purple-100 to-pink-100 border-purple-300'
                                : 'bg-white/40 backdrop-blur-sm hover:bg-white/80 border-gray-200/50 hover:border-purple-300'
                            }`}
                          >
